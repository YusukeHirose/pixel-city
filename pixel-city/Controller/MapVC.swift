...
class MapVC: UIViewController,UIGestureRecognizerDelegate {
    ...
    func addProgressLbl(){
        progressLbl = UILabel()
        progressLbl?.frame = CGRect(x: (screenSize.width / 2) - 120, y: 175, width: 240, height: 48)
...
//        print(frickrUrl(forApiKey: apiKey, withAnnotation: annotation, numberOfPhotos: 40))
    ...
    func retrieveUrls(forAnnotation annotation: DroppablePin, handler: @escaping(_ status: Bool) -> ()) {
        ...
        Alamofire.request(frickr(forApiKey: apiKey, withAnnotation: annotation, numberOfPhotos: 40)).responseJSON {(response) in
